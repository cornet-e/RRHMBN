...
server <- function(input, output, session) {
  ...
  results <- eventReactive(input$run, {
    ...
    data_excess_age_sex <- data_excess_age_sex %>%
      mutate(sex_label = ifelse(sex==1,"Hommes","Femmes"))

    df_quartile_all <- df_age_sex %>%
      filter(Tstop %in% c(1,5,10)) %>%
      group_by(age_quartile, Tstop) %>%
      summarise(surv.exp = mean(surv.exp, na.rm=TRUE), .groups="drop") %>% 
      mutate(horizon = paste0(Tstop, " ans"))

    df_quartile_male <- df_age_sex %>%
      filter(sex==1, Tstop %in% c(1,5,10)) %>%
      select(age_quartile, Tstop, surv.exp) %>%
      mutate(horizon = paste0(Tstop, " ans"))

    df_quartile_female <- df_age_sex %>%
      filter(sex==2, Tstop %in% c(1,5,10)) %>%
      select(age_quartile, Tstop, surv.exp) %>%
      mutate(horizon = paste0(Tstop, " ans"))
  ...
  output$plot_net_quartile_all <- renderPlotly({
    req(results())
    df <- results()$df_quartile_all
    p <- ggplot(df, aes(x = age_quartile, y = surv.exp, color = horizon, group = horizon)) +
      geom_line(linewidth = 1.3) + geom_point(size = 3) +
      labs(title = "Survie nette à 1 / 5 / 10 ans (tous sexes confondus)",
          x = "Quartile d’âge", y = "Survie nette (PP)") +
  ...
  output$plot_net_quartile_male <- renderPlotly({
    req(results())
    df <- results()$df_quartile_male
    p <- ggplot(df, aes(x = age_quartile, y = surv.exp, color = horizon, group = horizon)) +
      geom_line(linewidth = 1.3) + geom_point(size = 3) +
      labs(title = "Survie nette (Hommes) à 1 / 5 / 10 ans",
          x = "Quartile d’âge", y = "Survie nette (PP)") +
      theme_minimal(base_size = 14)
    ggplotly(p, tooltip = c("x","y","color"))
  })
  ...
  output$plot_net_quartile_female <- renderPlotly({
    req(results())
    df <- results()$df_quartile_female
    p <- ggplot(df, aes(x = age_quartile, y = surv.exp, color = horizon, group = horizon)) +
      geom_line(linewidth = 1.3) + geom_point(size = 3) +
      labs(title = "Survie nette (Femmes) à 1 / 5 / 10 ans",
          x = "Quartile d’âge", y = "Survie nette (PP)") +
      theme_minimal(base_size = 14)
    ggplotly(p, tooltip = c("x","y","color"))
  })
